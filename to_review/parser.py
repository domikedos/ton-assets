...
BLACK_LIST_JETTONS_TYPES = ["jetton_master", "jetton_wallet", "jetton_wallet_governed"]
...
def is_asset_to_black_list(types: list[str]):
    # add asset to blacklist if len(types) == 1 and this type is one of jetton types or if it has nft in type
    if len(types) == 1 and types[0] in BLACK_LIST_JETTONS_TYPES:
        return True

    for curr_type in types:
        if curr_type in BLACK_LIST_NFT_TYPES:
            return True
...
def get_asset_from_json_file(file: str, skip_addr_set: set[str]) -> dict[str, list[AssetData]]:
    ...
    with open(file, 'r') as f:
        ...
        for addr in data['addresses']:
            ...
            if is_asset_to_black_list(types):
...
def main():
    try:
        ...
        generate_html("to_review.html", assets["whitelist"])
